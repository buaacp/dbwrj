...
class MissionConfig:
    ...
    def __init__(self):
        ...
        self.socket_preinsert_axis_offset = float(get_param("socket_preinsert_axis_offset", -0.09))
        self.socket_insert_axis_offset = float(get_param("socket_insert_axis_offset", -0.02))
...
class BulbArmMission:
    ...
    def __init__(self):
        ...
        rospy.init_node("arm_control_bulb", anonymous=True)
    ...
    def step_state(self, state, control_dt):
        ...
        if state == 1:
            return self.track_alignment_state(
                state,
                self.bulb_target("PICK_ALIGN"),
                open_cmd,
        ...
        if state == 4:
            # Mirror state1: align above the socket centerline first.  Do not
            # push the bulb into the socket before UAV switches to state5.
            return self.track_alignment_state(
                state,
                self.socket_preinsert_target("TRANSFER_TO_SOCKET"),
                closed_cmd,
                require_stable=False,
            )

        if state == 5:
            # Mirror state2: after state4 has converged on the centerline,
            # move from the preinsert point toward the insert point.
            return self.track_alignment_state(state, self.socket_insert_target("FINE_ALIGN_SOCKET"), closed_cmd)
    ...
    def track_alignment_state(self, state, target, gripper, require_stable=True):
        result = self.track_target(target, gripper)
        if self.config.safe_demo_mode:
            result.angular = zero_arm_control(self.config.servo_ids)
            result.ready_state = state
            return result
        reached = self.target_stable(result) if require_stable else self.target_reached(result)
        if reached:
            result.ready_state = state
        return result
    ...
    def socket_preinsert_target(self, label):
        """Target above the socket centerline before insertion."""
        return center_axis_target(
            "socket",
            self.config.socket_axis_local,
            self.config.socket_center_axis_offset + self.config.socket_preinsert_axis_offset,
            label,
        )

    def socket_insert_target(self, label):
        """Target the insertion point along the socket centerline."""
        return center_axis_target(
            "socket",
            self.config.socket_axis_local,
            self.config.socket_center_axis_offset + self.config.socket_insert_axis_offset,
            label,
        )
