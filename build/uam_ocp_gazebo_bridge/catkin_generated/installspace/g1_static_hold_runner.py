...
from uam_ocp_gazebo_bridge.trajectory_profile import (
    required_duration,
    sample_profile,
    takeoff_abort_reason,
)
...
ABORT_TAKEOFF_NO_ASCENT = "TAKEOFF_NO_ASCENT"
...
class MonitorFreshness(object):
    """基于 ``StateMonitor`` 的单一 MAVROS state 新鲜度判定器。

    旧版本额外订阅了一次 ``/mavros/state``。这会产生两套互不一致的
    接收时间戳：StateMonitor 已能确认 OFFBOARD/arming，而第二个 subscriber
    却可能停在首次消息，从而在起飞中误报 ``MAVROS_STATE_STALE``。

    本类**不创建任何 ROS subscriber**。它仅观察 ``StateMonitor.receive_times()``
    中 state topic 的接收时间标记：标记变化即代表 StateMonitor 收到了新状态。
    控制循环在观察到标记变化的当前仿真时刻记录 ``last_state_sim``，因此可在
    不修改旧 StateMonitor 公共 API 的前提下，得到一致的 sim-time freshness。
    """

    def __init__(self, monitor, watchdog_cfg):
        ...
        self._state_stale_checks = 0
        self._source_logged = False

    @staticmethod
    def _candidate_state_key(receive_times, preferred_key=None):
        """Find the MAVROS state key without assuming one YAML naming style."""
        if preferred_key in receive_times:
            return preferred_key
        candidates = []
        for key in receive_times:
            normalized = str(key).lower().replace('/', '_').replace('-', '_')
            score = 0
            if normalized in ('mavros_state', 'state', 'vehicle_state'):
                score += 100
            if 'state' in normalized:
                score += 20
            if 'mavros' in normalized:
                score += 10
    ...
    def update(self, wall_now, sim_now):
        """Observe StateMonitor receive markers and update same-domain ages.

        ``receive_times`` may use a project-specific wall-clock representation.
        Its numerical value is therefore used only as a *change token*; it is
        never subtracted from ``time.monotonic()``.  This avoids cross-domain
        arithmetic even if an older StateMonitor used ``time.time()``.
        """
        raw_receive = dict(self.monitor.receive_times() or {})
        self._state_key = self._candidate_state_key(raw_receive, self._state_key)
        token = raw_receive.get(self._state_key) if self._state_key else None

        if token is not None and token != self._last_state_token:
            self._last_state_token = token
            self._last_state_wall = float(wall_now)
            self._last_state_sim = float(sim_now)
            self._state_count += 1
            if not self._source_logged:
                rospy.loginfo(
                    "G1 MAVROS freshness source=StateMonitor receive_key=%s",
                    self._state_key,
                )
                self._source_logged = True

        # /clock freshness is derived directly from the simulation timestamp
        # seen by the main loop. A change in rospy.get_time() means /clock is
        # progressing; no extra Clock subscriber is needed.
        if (self._last_clock_sim is None or
                float(sim_now) != float(self._last_clock_sim)):
            self._last_clock_sim = float(sim_now)
            self._last_clock_wall = float(wall_now)

        state_age_wall = (
            None if self._last_state_wall is None
            else float(wall_now) - self._last_state_wall)
        state_age_sim = (
            None if self._last_state_sim is None
            else float(sim_now) - self._last_state_sim)
        clock_age_wall = (
            None if self._last_clock_wall is None
            else float(wall_now) - self._last_clock_wall)

        return {
            "source": "StateMonitor",
            "state_receive_key": self._state_key,
            "state_last_wall_s": self._last_state_wall,
            "state_last_sim_s": self._last_state_sim,
            "state_age_wall_s": state_age_wall,
            "state_age_sim_s": state_age_sim,
            "state_receive_count": int(self._state_count),
            "clock_last_wall_s": self._last_clock_wall,
            "clock_last_sim_s": self._last_clock_sim,
            "clock_age_wall_s": clock_age_wall,
        }

    def state_receive_count(self):
        return int(self._state_count)

    def legacy_receive_times(self, wall_now):
        """Return legacy receive times with only MAVROS-state stale masked.

        The existing SafetySupervisor may use a short wall-time state threshold.
        G1 owns MAVROS freshness through this class, so the legacy supervisor
        must not independently report a false state stale. Other topics retain
        their original receive timestamps and remain protected.
        """
        receive = dict(self.monitor.receive_times() or {})
        key = self._candidate_state_key(receive, self._state_key)
        if key is not None:
            receive[key] = float(wall_now)
        return receive

    def evaluate(self, wall_now, sim_now, snapshot, offboard_confirmed,
                 armed_confirmed, confirmation_state_count):
        """Return ``(reason, diagnostics)`` for state/clock freshness.

        The simulation-time state age is the main stale criterion. Wall time is
        only a conservative 20 s-style hard fallback. Explicit newer state
        updates showing OFFBOARD/arming loss take priority over age checks.
        """
        info = self.update(wall_now, sim_now)

        # A frozen /clock is distinct from a missing MAVROS state update.
        if (info["clock_age_wall_s"] is not None and
                info["clock_age_wall_s"] > self.clock_stale_wall_s):
        ...
        # Only examine an explicit loss after a state update newer than the
        # update that confirmed OFFBOARD + arming.
        if info["state_receive_count"] > int(confirmation_state_count):
            if not bool(snapshot["offboard"]):
                return ABORT_OFFBOARD_LOST, info
            if not bool(snapshot["armed"]):
        ...
        if sim_stale or hard_wall_stale:
            self._state_stale_checks += 1
        else:
            self._state_stale_checks = 0

        if self._state_stale_checks >= self.state_stale_consecutive_checks:
...
class G1Runner(object):
    ...
    def __init__(self, cfg, interfaces, scenario, root):
        ...
        # MAVROS state freshness must use StateMonitor as its only data source.
        # Do not create a second /mavros/state subscriber here.
        self.runtime_freshness = MonitorFreshness(
            self.monitor, cfg.get("watchdog", {}))
        ...
        self.takeoff_duration = max(float(takeoff.get("transition_duration_s", 8.0)), 1e-3)
        ...
        self.takeoff_no_ascent_check_after_s = float(takeoff.get("no_ascent_check_after_s", 2.0))
        self.takeoff_min_ascent_after_check_m = float(takeoff.get("min_ascent_after_check_m", 0.08))
        ...
        self.last_freshness = {}
        self.takeoff_setpoint_logged = False
    ...
    def _capture_initial_reference(self, snapshot, wall_now):
        ...
        rospy.loginfo("G1 initial telemetry ready")
        rospy.loginfo(
    ...
    def _transition(self, old_state, new_state, sim_now, snapshot):
        ...
        rospy.loginfo("G1 state: %s -> %s", old_state, new_state)
        ...
        if new_state == "ARM_NEUTRALIZE":
            ...
            self._log_takeoff_setpoint_interface()
    ...
    def _log_takeoff_setpoint_interface(self):
        """Print the MAVROS setpoint message semantics once per run."""
        if self.takeoff_setpoint_logged:
            return
        info = self.hover.describe_position_setpoint(self.hold_position, self.hold_yaw)
        rospy.loginfo(
            "G1 takeoff setpoint interface:\n"
            "  message_type=%s\n"
            "  coordinate_frame=%s (%s)\n"
            "  type_mask=%s\n"
            "  requested_position_enu=%s\n"
            "  outgoing_position=%s\n"
            "  requested_yaw_rad=%.6f",
            info["message_type"],
            info["coordinate_frame_name"],
            info["coordinate_frame"],
            info["type_mask"],
            info["requested_position_enu"],
            info["outgoing_position"],
            info["requested_yaw_rad"],
        )
        self.takeoff_setpoint_logged = True
    ...
    def _set_abort(self, reason, state, sim_now, snapshot, reference=None):
        ...
        if self.abort is None:
            ...
            diagnostics = self._takeoff_tracking_diagnostics(
                state, sim_now, snapshot, reference)
            if diagnostics is not None:
                self.abort["takeoff_tracking_diagnostics"] = diagnostics
                rospy.logerr(
                    "G1 takeoff tracking diagnostics:\n"
                    "  sim_elapsed_s=%.3f\n"
                    "  takeoff_duration_s=%.3f\n"
                    "  actual_position=%s\n"
                    "  reference_position=%s\n"
                    "  position_error_xyz=%s\n"
                    "  position_error_norm_m=%.3f\n"
                    "  actual_velocity=%s\n"
                    "  speed_mps=%.3f\n"
                    "  initial_position=%s\n"
                    "  final_hold_position=%s\n"
                    "  offboard=%s\n"
                    "  armed=%s",
                    diagnostics["sim_elapsed_s"],
                    diagnostics["takeoff_duration_s"],
                    diagnostics["actual_position"],
                    diagnostics["reference_position"],
                    diagnostics["position_error_xyz"],
                    diagnostics["position_error_norm_m"],
                    diagnostics["actual_velocity"],
                    diagnostics["speed_mps"],
                    diagnostics["initial_position"],
                    diagnostics["final_hold_position"],
                    diagnostics["offboard"],
                    diagnostics["armed"],
                )
            rospy.logerr("G1 ABORT: %s", reason)

    def _takeoff_tracking_diagnostics(self, state, sim_now, snapshot, reference):
        """Return detailed takeoff context for TAKEOFF_* abort reasons."""
        if state != "TAKEOFF_TRANSITION" or snapshot is None or reference is None:
            return None
        elapsed = float(sim_now - self.state_start_sim)
        actual = np.asarray(snapshot["position"], dtype=float)
        velocity = np.asarray(snapshot["velocity"], dtype=float)
        ref = np.asarray(reference.position, dtype=float)
        err = actual - ref
        initial = (self.takeoff_start_position
                   if self.takeoff_start_position is not None else self.initial_position)
        return {
            "sim_elapsed_s": elapsed,
            "takeoff_duration_s": float(self.takeoff_duration),
            "actual_position": actual.tolist(),
            "reference_position": ref.tolist(),
            "position_error_xyz": err.tolist(),
            "position_error_norm_m": float(np.linalg.norm(err)),
            "actual_velocity": velocity.tolist(),
            "speed_mps": float(np.linalg.norm(velocity)),
            "initial_position": (
                np.asarray(initial, dtype=float).tolist()
                if initial is not None else None),
            "final_hold_position": self.hold_position.tolist(),
            "actual_ascent_m": (
                float(actual[2] - np.asarray(initial, dtype=float)[2])
                if initial is not None else None),
            "reference_ascent_m": (
                float(ref[2] - np.asarray(initial, dtype=float)[2])
                if initial is not None else None),
    ...
    def _legacy_safety_reason(self, wall_now, state, snapshot, reference):
        ...
        """
        # MonitorFreshness 单独负责 MAVROS heartbeat。这里只屏蔽旧
        # SafetySupervisor 对 MAVROS state 的短 wall-time stale；pose/imu/
        # joint_state 等其他 topic 的 legacy freshness 逻辑保持不变。
        receive = self.runtime_freshness.legacy_receive_times(wall_now)
    ...
    def _safety_reason(self, wall_now, sim_now, state, snapshot, reference):
        ...
        if state == "TAKEOFF_TRANSITION":
            ...
            if initial is not None:
                reason = takeoff_abort_reason(
                    snapshot["position"],
                    reference.position,
                    initial,
                    elapsed,
                    self.takeoff_no_ascent_check_after_s,
                    self.takeoff_min_ascent_after_check_m,
                    self.takeoff_transition_error_m,
                )
                if reason:
                    return reason

        freshness_reason, freshness = self.runtime_freshness.evaluate(
            wall_now, sim_now, snapshot, self.offboard_confirmed,
            self.armed_confirmed, self.confirmation_state_receive_count)
    ...
    def _request_offboard_and_arm(self, wall_now, snapshot):
        ...
        try:
            if not self.offboard_confirmed:
                ...
                rospy.loginfo("G1 requested OFFBOARD")
                ...
                rospy.loginfo("G1 requested arming")
        except Exception as exc:
    ...
    def _record(self, sim_now, start_wall, state, snapshot, reference):
        ...
        takeoff_transition_error = (
            position_error_active
            if reference.position_kind == "TAKEOFF_TRANSITION" else float("nan"))
        ...
        row = {
            ...
            "takeoff_transition_error_m": takeoff_transition_error,
    ...
    def _finish(self, status, metrics):
        ...
        metadata = {
            ...
            "takeoff_reference": {
                ...
                "transition_position_error_m": self.takeoff_transition_error_m,
                "no_ascent_check_after_s": self.takeoff_no_ascent_check_after_s,
                "min_ascent_after_check_m": self.takeoff_min_ascent_after_check_m,
            },
        ...
        rospy.loginfo("G1 %s: %s; files: %s", self.scenario, status, self.run_directory)
        return status
    ...
    def run(self):
        ...
        rospy.loginfo("G1 state: IDLE -> PRESTREAM_SETPOINTS")
        ...
        while not rospy.is_shutdown() and state != "COMPLETE":
            ...
            if not self.startup_gate.ready():
                ...
                if not waiting_logged:
                    rospy.loginfo("G1 waiting for initial telemetry: missing=%s",
...
def main():
    ...
    rospy.init_node("g1_static_hold_runner", log_level=rospy.INFO)
