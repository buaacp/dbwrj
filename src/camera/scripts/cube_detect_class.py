...
class PolygonDetector:
    def __init__(self, min_distance=50, epsilon_ratio=0.01):
        """
        :param min_distance: 相邻角点最小间距（像素）
        :param epsilon_ratio: 逼近精度比例，越小越精细
        """
        self.min_distance = min_distance
        self.epsilon_ratio = epsilon_ratio

    def img_process_canny2max_contour(self, canny_img):
        """从 Canny 图像中提取面积最大的轮廓"""
        contours_info = cv2.findContours(canny_img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    def approx_polygone(self, contour):
        """多边形逼近"""
        if contour is None:
            return None
    ...
    def get_poly_points(self, approx_poly):
        """过滤掉距离过近的点，自适应返回角点数量"""
        if approx_poly is None:
            return np.array([])
        ...
        filtered = [points[0]]
        for p in points[1:]:
            if np.linalg.norm(p - filtered[-1]) > self.min_distance:
                filtered.append(p)
        # 闭合检查
        if np.linalg.norm(filtered[0] - filtered[-1]) < self.min_distance:
            filtered.pop(-1)
        return np.array(filtered)

    def detect_polygon(self, canny_img):
        """主函数：直接从 Canny 图识别多边形"""
        max_contour = self.img_process_canny2max_contour(canny_img)
        poly = self.approx_polygone(max_contour)
        poly_points = self.get_poly_points(poly)

        # print(f"检测到多边形点数: {len(poly_points)}")
        return poly_points
    def get_polygon_center(self,poly_points):
        if len(poly_points) < 3:
            return None  # 少于3个点无法构成多边形
        ...
        area *= 0.5
        if abs(area) < 1e-6:
            return np.mean(pts, axis=0).astype(int)  # 面积太小用平均值代替
        cx /= (6 * area)
        cy /= (6 * area)
