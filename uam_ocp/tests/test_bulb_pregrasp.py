...
class TestBulbPregrasp(unittest.TestCase):
 ...
 def test_three_strategies(self):
  for name in ("arm_dominant","uav_dominant","whole_body"):
   ...
   self.assertEqual(len(solution.costs_pass_1),solution.iterations_pass_1+1)
   self.assertEqual(len(solution.costs_pass_2),solution.iterations_pass_2+1)
   ...
   self.assertGreater(solution.dynamics_gap_penalty_weight,0.0)
   self.assertGreater(solution.diagnostics_pass_1["dynamics_gap_max"][0],1e-8)
   self.assertLess(solution.diagnostics_pass_1["dynamics_gap_max"][-1],1e-8)
   self.assertEqual(solution.costs_pass_2,solution.diagnostics_pass_2["diagnostic_total_cost"])
   self.assertEqual(solution.costs_pass_1,solution.diagnostics_pass_1["diagnostic_total_cost"])
 def test_pass_two_cost_plot_label(self):
  text=(Path(__file__).resolve().parents[1]/"uam_ocp"/"visualization.py").read_text()
  self.assertIn("BoxFDDP pass 2 diagnostic total cost convergence",text)
  self.assertIn("Pass-2 BoxFDDP iteration",text)
  result_text=(Path(__file__).resolve().parents[1]/"uam_ocp"/"bulb_pregrasp_results.py").read_text()
  self.assertIn("optimization_cost_convergence_pass1.png",result_text)
  self.assertIn("optimization_cost_convergence_pass2.png",result_text)
  self.assertIn("optimization_cost_convergence_two_passes.png",result_text)
  self.assertIn("optimization_pass1_diagnostics.png",result_text)
  self.assertIn("dynamics_gap_penalty_weight",result_text)
